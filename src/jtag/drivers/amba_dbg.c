...
// TODO: 创建一个amba dummy 适配器, 用来从文件中读取
...
struct stlink_usb_handle {
    /** */
    struct amba_dbg_backend *backend;       // 后端实现： debug bus 或者 dummy bus
    /** */
    union {
...
// TODO: 实现一个不检查错误的传输函数
// 第一种方法：直接读 debug bus
// 第二种办法：向 dummy debug bus 发送一个读请求
...
static void amba_dbg_dump_core_frequency(const struct speed_map *map, unsigned int map_size);
...
static int jtag_libusb_bulk_transfer_n(struct libusb_device_handle *dev_handle,
                                       ...
                                       int                          timeout)
{
    ...
    for (size_t i = 0; i < n_transfers; ++i) {
        ...
        if (!transfers[i].transfer) {
            for (size_t j = 0; j < i; ++j) libusb_free_transfer(transfers[j].transfer);

            LOG_DEBUG("ERROR, failed to alloc usb transfers");
            for (size_t k = 0; k < n_transfers; ++k) transfers[k].retval = LIBUSB_ERROR_NO_MEM;
            return ERROR_FAIL;
        }
    }

    for (size_t i = 0; i < n_transfers; ++i) {
        libusb_fill_bulk_transfer(transfers[i].transfer,
                                  dev_handle,
                                  transfers[i].ep,
                                  transfers[i].buf,
                                  transfers[i].size,
                                  sync_transfer_cb,
                                  &transfers[i].completed,
                                  timeout);
        ...
        if (retval < 0) {
            ...
            /* Probably no point continuing to submit transfers once a submission
             * fails. As a result, tag all remaining transfers as errors.
             */
            for (size_t j = i; j < n_transfers; ++j) transfers[j].retval = retval;
...
static int stlink_usb_xfer_v1_get_status(void *handle)
{
    ...
    ret = jtag_libusb_bulk_read(h->usb_backend_priv.fd,
                                h->rx_ep,
                                (char *)h->cmdbuf,
                                13,
                                STLINK_READ_TIMEOUT,
...
static int stlink_usb_xfer_rw(void *handle, int cmdsize, const uint8_t *buf, int size)
{
    ...
    return jtag_libusb_bulk_transfer_n(h->usb_backend_priv.fd,
                                       transfers,
                                       n_transfers,
...
static int stlink_usb_xfer_rw(void *handle, int cmdsize, const uint8_t *buf, int size)
{
    ...
    ret = jtag_libusb_bulk_write(h->usb_backend_priv.fd,
                                 h->tx_ep,
                                 (char *)h->cmdbuf,
                                 cmdsize,
                                 STLINK_WRITE_TIMEOUT,
...
static int stlink_tcp_send_cmd(void *handle, int send_size, int recv_size, bool check_tcp_status)
{
    ...
    int sent_size =
        send(h->tcp_backend_priv.fd, (void *)h->tcp_backend_priv.send_buf, send_size, 0);
...
static int stlink_tcp_xfer_noerrcheck(void *handle, const uint8_t *buf, int size)
{
    ...
    memset(&h->tcp_backend_priv.send_buf[1],
           0,
           3); /* reserved for alignment and future use, must be zero */
    h_u32_to_le(&h->tcp_backend_priv.send_buf[4], h->tcp_backend_priv.connect_id);
    /* tcp_backend_priv.send_buf[8..23] already contains the constructed stlink
     * command */
    h->tcp_backend_priv.send_buf[24] = h->direction;
    memset(&h->tcp_backend_priv.send_buf[25],
           0,
           3); /* reserved for alignment and future use, must be zero */
    ...
    if (h->direction != h->tx_ep) {
         ...
         * tcp_backend_priv.recv_buf[4] to handle->cmd_buff which are the same, so
         * let's avoid unnecessary copying */
...
/** Issue an STLINK command via USB transfer, with retries on any wait status
   responses.

    Works for commands where the STLINK_DEBUG status is returned in the first
    byte of the response packet. For SWIM a SWIM_READSTATUS is requested
   instead.
...
static int stlink_cmd_allow_retry(void *handle, const uint8_t *buf, int size)
{
    ...
    while (1) {
        ...
        if (h->st_mode == STLINK_MODE_DEBUG_SWIM) {
            ...
            res = ERROR_OK;  // TODO: implement swim status check
            if (res != ERROR_OK)
                return res;
        }
        ...
        if (res == ERROR_WAIT && retries < MAX_WAIT_RETRIES) {
            ...
            LOG_DEBUG(
                "stlink_cmd_allow_retry ERROR_WAIT, retry %d, delaying %u "
                "microseconds",
                retries,
                delay_us);
            usleep(delay_us);
...
/** */
static int amba_dbg_sdk_version(void *handle)
{
    int                       res;
    uint32_t                  flags;
    uint16_t                  version;
    ...
    LOG_INFO("Kernel: %d.%d", 3, 0);
    LOG_INFO("IAV.  : %d.%d", 3, 0);
    LOG_INFO("uCode : %s", "260201_080000");
...
/**
 * @brief
 * 调试器固件中的一个函数，用于退出ST-LINK当前的操作模式，让其返回到一个已知状态。让我详细解释：
 *
 * @param handle
 * @return int
 */
static int stlink_usb_exit_mode(void *handle)
{
    ...
    // 首先获取ST-LINK当前的工作模式，返回一个mode值（8位无符号整数）
...
static int stlink_usb_init_mode(void *handle, bool connect_under_reset, int initial_interface_speed)
{
    ...
    /* set the speed before entering the mode, as the chip discovery phase should
     * be done at this speed too */
    if (emode == STLINK_MODE_DEBUG_JTAG) {
        if (h->version.flags & STLINK_F_HAS_JTAG_SET_FREQ) {
            amba_dbg_dump_core_frequency(stlink_khz_to_speed_map_jtag,
                                         ARRAY_SIZE(stlink_khz_to_speed_map_jtag));
            stlink_speed(h, initial_interface_speed, false);
        }
    } else if (emode == STLINK_MODE_DEBUG_SWD) {
        if (h->version.flags & STLINK_F_HAS_SWD_SET_FREQ) {
            amba_dbg_dump_core_frequency(stlink_khz_to_speed_map_swd,
    ...
    if (h->version.jtag_api == STLINK_JTAG_API_V3 &&
        ...
        amba_dbg_dump_core_frequency(map, ARRAY_SIZE(map));
        stlink_speed(h, initial_interface_speed, false);
    }

    /* preliminary SRST assert:
     * We want SRST is asserted before activating debug signals (mode_enter).
     * As the required mode has not been set, the adapter may not know what pin to
     * use. Tested firmware STLINK v2 JTAG v29 API v2 SWIM v0 uses T_NRST pin by
     * default Tested firmware STLINK v2 JTAG v27 API v2 SWIM v6 uses T_NRST pin
     * by default after power on, SWIM_RST stays unchanged */
    ...
    /* assert SRST again: a little bit late but now the adapter knows for sure
     * what pin to use */
 ...
 * @return int 
 */
// TODO: 这里的 IDCODE 是硬编码的，实际应用中需要读取芯片的 OTP
static int amba_dbg_otpcode(void *handle, uint32_t *idcode)
{
    assert(handle);

    *idcode = 0x1BA01477;    // NOTE: 这是从 STM32F103CBT6 芯片上读取到的 IDCODE 值

    LOG_WARNING("OTP 查询功能尚不支持");
...
static int stlink_usb_step(void *handle)
{
    ...
    if (h->version.jtag_api != STLINK_JTAG_API_V1) {
        /* TODO: this emulates the v1 api, it should really use a similar auto mask
         * isr that the Cortex-M3 currently does. */
...
/** */
static int stlink_usb_read_mem8(
    void *handle, uint8_t ap_num, uint32_t csw, uint32_t addr, uint16_t len, uint8_t *buffer)
{
    int                       res;
...
/** */
static int stlink_usb_write_mem8(
    void *handle, uint8_t ap_num, uint32_t csw, uint32_t addr, uint16_t len, const uint8_t *buffer)
{
    int                       res;
    struct stlink_usb_handle *h = handle;
...
/** */
static int stlink_usb_read_mem16(
    void *handle, uint8_t ap_num, uint32_t csw, uint32_t addr, uint16_t len, uint8_t *buffer)
{
    int                       res;
    struct stlink_usb_handle *h = handle;
...
/** */
static int stlink_usb_write_mem16(
    void *handle, uint8_t ap_num, uint32_t csw, uint32_t addr, uint16_t len, const uint8_t *buffer)
{
    int                       res;
    struct stlink_usb_handle *h = handle;
...
/** */
static int stlink_usb_read_mem32(
    void *handle, uint8_t ap_num, uint32_t csw, uint32_t addr, uint16_t len, uint8_t *buffer)
{
    int                       res;
    struct stlink_usb_handle *h = handle;
...
static int stlink_usb_read_mem32_noaddrinc(
    void *handle, uint8_t ap_num, uint32_t csw, uint32_t addr, uint16_t len, uint8_t *buffer)
{
    struct stlink_usb_handle *h = handle;
...
static int stlink_usb_write_mem32_noaddrinc(
    void *handle, uint8_t ap_num, uint32_t csw, uint32_t addr, uint16_t len, const uint8_t *buffer)
{
    struct stlink_usb_handle *h = handle;
                                  ...
                                  uint8_t *buffer)
{
    ...
    while (count) {
        bytes_remaining =
            (size != 1) ? stlink_max_block_size(h->max_mem_packet, addr) : stlink_usb_block(h);
        ...
        if (size != 1) {
            ...
            if (bytes_remaining & (size - 1))
                retval =
                    stlink_usb_read_ap_mem(handle, ap_num, csw, addr, 1, bytes_remaining, buffer);
            else if (size == 2)
...
static int stlink_usb_read_mem(
    void *handle, uint32_t addr, uint32_t size, uint32_t count, uint8_t *buffer)
{
    return stlink_usb_read_ap_mem(handle,
                                  STLINK_HLA_AP_NUM,
                                  STLINK_HLA_CSW,
                                  addr,
                                  size,
                                   ...
                                   const uint8_t *buffer)
{
    ...
    while (count) {
        ...
        if (size != 1) {
            ...
            if (bytes_remaining & (size - 1))
                retval =
                    stlink_usb_write_ap_mem(handle, ap_num, csw, addr, 1, bytes_remaining, buffer);
            else if (size == 2)
...
static int stlink_usb_write_mem(
    void *handle, uint32_t addr, uint32_t size, uint32_t count, const uint8_t *buffer)
{
    return stlink_usb_write_ap_mem(handle,
                                   STLINK_HLA_AP_NUM,
                                   STLINK_HLA_CSW,
                                   addr,
                                   size,
...
static int stlink_speed_swd(void *handle, int khz, bool query)
{
    ...
    if (!query) {
        int result =
            stlink_usb_set_swdclk(h, stlink_khz_to_speed_map_swd[speed_index].speed_divisor);
        if (result != ERROR_OK) {
...
static int stlink_speed_jtag(void *handle, int khz, bool query)
{
    ...
    if (!query) {
        int result =
            stlink_usb_set_jtagclk(h, stlink_khz_to_speed_map_jtag[speed_index].speed_divisor);
        if (result != ERROR_OK) {
            LOG_ERROR("Unable to set adapter speed");
            return khz;
        }
...
static int stlink_get_com_freq(void *handle, bool is_jtag, struct speed_map *map)
{
    ...
    for (i = size; i < STLINK_V3_MAX_FREQ_NB; i++) map[i].speed = 0;
 ...
 * 0x32, 0x00 ... this contains the length (0x32 = 50), the type (0x3 =
 * DT_STRING) and the serial in unicode format the serial part is: 0x0035,
 * 0x0037, 0x0046, 0x0046, 0x0037, 0x0032 ... >>  57FF72 ... this format could
 * be read correctly by 'libusb_get_string_descriptor_ascii' so this case is
 * managed by libusb_helper::string_descriptor_equal
 *  - the buggy DFU is not doing any unicode conversion and returns a raw serial
 * data in the descriptor 0x1a, 0x03, 0x57, 0x00, 0xFF, 0x00, 0x72, 0x00 ...
 *            >>    57          FF          72       ...
 *    based on the length (0x1a = 26) we could easily decide if we have to fixup
 * the serial and then we have just to convert the raw data into printable
 * characters using sprintf
 */
                                             ...
                                             struct libusb_device_descriptor *dev_desc)
{
    ...
    usb_retval = libusb_get_string_descriptor(device, 0, 0, desc_serial, sizeof(desc_serial));

    if (usb_retval < LIBUSB_SUCCESS) {
        LOG_ERROR("libusb_get_string_descriptor() failed: %s(%d)",
                  libusb_error_name(usb_retval),
                  usb_retval);
        return NULL;
    } else if (usb_retval < 4) {
        /* the size should be least 4 bytes to contain a minimum of 1 supported
         * LANGID */
...
static int stlink_usb_usb_open(void *handle, struct hl_interface_param *param)
{
     ...
     */
    do {
        if (jtag_libusb_open(param->vid,
                             param->pid,
                             NULL,
                             &h->usb_backend_priv.fd,
                             stlink_usb_get_alternate_serial) != ERROR_OK) {
            // 无法打开 ST-LINK
            LOG_ERROR("无法通过 USB 打开 ST-LINK 设备");
            return ERROR_FAIL;
        }
        ...
        err = amba_dbg_sdk_version(h);

        if (err == ERROR_OK) {
            break;
        } else if (h->version.stlink == 1 || retry_count == 0) {
...
static int stlink_tcp_open(void *handle, struct hl_interface_param *param)
{
    ...
    int optval = 1;
    if (setsockopt(h->tcp_backend_priv.fd,
                   IPPROTO_TCP,
                   TCP_NODELAY,
                   (const void *)&optval,
                   sizeof(int)) == -1) {
     ...
     * stlink-server to crash in windows: select a safe default value (1K) */
    ...
    /* get stlink version */
    return amba_dbg_sdk_version(h);
}

static struct amba_dbg_backend amba_dbg_bus_backend = {
...
static struct amba_dbg_backend amba_dbg_dummy_backend = {
...
static int dbg_bus_open(struct hl_interface_param *param, enum stlink_mode mode, void **fd)
{
    struct stlink_usb_handle *h;

    LOG_DEBUG("打开调试总线, 模式 %u", mode);
    ...
    for (unsigned int i = 0; param->vid[i]; i++) {
        LOG_DEBUG("transport: %d vid: 0x%04x pid: 0x%04x 串口: %s",
                  h->st_mode,
                  param->vid[i],
                  param->pid[i],
    ...
    if (param->use_stlink_tcp)
        h->backend = &amba_dbg_dummy_backend;
    else
        h->backend = &amba_dbg_bus_backend;
...
{
    return dbg_bus_open(param, stlink_get_mode(param->transport), fd);
}
                               ...
                               uint16_t              *prescaler)
{
    ...
    unsigned int max_trace_freq =
        (h->version.stlink >= 3) ? STLINK_V3_TRACE_MAX_HZ : STLINK_TRACE_MAX_HZ;

    /* Only concern ourselves with the frequency if the STlink is processing it.
     */
    if (*trace_freq > max_trace_freq) {
        LOG_ERROR("ST-LINK doesn't support SWO frequency higher than %u", max_trace_freq);
        return ERROR_FAIL;
    }
...
/** */
static int stlink_read_dap_register(void          *handle,
...
/** */
struct hl_layout_api amba_dbg_layout_api = {
    /** */
    .open = stlink_usb_hl_open,
    /** */
    .close = stlink_close,
    /** */
    .idcode = amba_dbg_otpcode,
    /** */
...
/** */
static int amba_dbg_op_connect(struct adiv5_dap *dap)
{
    uint32_t idcode;
    int      retval;
    ...
    /* Check if we should reset srst already when connecting, but not if
     * reconnecting. */
    if (!dap->do_reconnect) {
    ...
    for (unsigned int i = 0; i <= DP_APSEL_MAX; i++) last_csw_default[i] = 0;
    ...
    retval = amba_dbg_otpcode(stlink_dap_handle, &idcode);
    if (retval == ERROR_OK)
...
static int stlink_dap_check_reconnect(struct adiv5_dap *dap)
{
    ...
    return amba_dbg_op_connect(dap);
}

/** */
static int amba_dbg_op_send_sequence(struct adiv5_dap *dap, enum swd_special_seq seq)
{
    /* Ignore the request */
    return ERROR_OK;
}
...
static int stlink_dap_dp_read(struct adiv5_dap *dap, unsigned int reg, uint32_t *data)
{
    ...
    if (stlink_dap_handle->version.flags & STLINK_F_QUIRK_JTAG_DP_READ &&
        ...
        retval = stlink_read_dap_register(stlink_dap_handle, STLINK_DEBUG_PORT_ACCESS, reg, &dummy);
        if (retval == ERROR_OK)
        ...
        retval = stlink_read_dap_register(stlink_dap_handle, STLINK_DEBUG_PORT_ACCESS, reg, data);
    }
...
static int stlink_dap_dp_write(struct adiv5_dap *dap, unsigned int reg, uint32_t data)
{
    ...
    retval = stlink_write_dap_register(stlink_dap_handle, STLINK_DEBUG_PORT_ACCESS, reg, data);
    return retval;
}
...
static int stlink_dap_ap_read(struct adiv5_ap *ap, unsigned int reg, uint32_t *data)
{
    ...
    data                       = data ? data : &dummy;
    retval                     = stlink_read_dap_register(stlink_dap_handle, ap->ap_num, reg, data);
...
static int stlink_dap_ap_write(struct adiv5_ap *ap, unsigned int reg, uint32_t data)
{
    ...
    retval = stlink_write_dap_register(stlink_dap_handle, ap->ap_num, reg, data);
    dap->stlink_flush_ap_write = true;
    return retval;
}

/** */
static int amba_dbg_op_queue_ap_abort(struct adiv5_dap *dap, uint8_t *ack)
{
...
static int stlink_usb_misc_rw_segment(void                   *handle,
                                      ...
                                      unsigned int            items)
{
    ...
    while (!IS_ALIGNED(cmd_index, 4)) buf[cmd_index++] = 0;
...
static int stlink_usb_buf_rw_segment(void *handle, const struct dap_queue *q, unsigned int count)
{
    ...
    switch (q[0].cmd) {
        ...
        case CMD_MEM_AP_WRITE32:
            for (unsigned int i = 0; i < count; i++) h_u32_to_le(&buf[4 * i], q[i].mem_ap.data);
            if (count > 1 && q[0].mem_ap.addr == q[1].mem_ap.addr)
                return stlink_usb_write_mem32_noaddrinc(stlink_dap_handle,
                                                        ap_num,
                                                        csw,
                                                        addr,
                                                        bufsize,
        ...
        case CMD_MEM_AP_READ32:
            if (count > 1 && q[0].mem_ap.addr == q[1].mem_ap.addr)
                retval = stlink_usb_read_mem32_noaddrinc(stlink_dap_handle,
                                                         ap_num,
                                                         csw,
                                                         addr,
                                                         bufsize,
...
static int stlink_usb_count_misc_rw_queue(void                   *handle,
                                          const struct dap_queue *q,
                                          unsigned int            len,
                                          unsigned int           *pkt_items)
{
    ...
    uint32_t                  ap_num = DP_APSEL_INVALID;
    unsigned int              misc_max_items =
        (h->version.stlink == 2) ? STLINK_V2_RW_MISC_SIZE : STLINK_V3_RW_MISC_SIZE;
     ...
     */
    if (h->backend == &amba_dbg_dummy_backend && h->tcp_backend_priv.version.api < 3)
        return 0;
...
static int stlink_usb_count_buf_rw_queue(const struct dap_queue *q, unsigned int len)
{
    ...
    for (unsigned int i = 1; i < len; i++)
        if (q[i].cmd != q[0].cmd || q[i].mem_ap.ap != q[0].mem_ap.ap ||
            q[i].mem_ap.csw != q[0].mem_ap.csw || q[i].mem_ap.addr != q[i - 1].mem_ap.addr + incr)
            return i;
...
static int stlink_dap_run_finalize(struct adiv5_dap *dap)
{
    ...
    if (ctrlstat & SSTICKYERR) {
        if (stlink_dap_handle->st_mode == STLINK_MODE_DEBUG_JTAG)
            retval =
                stlink_dap_dp_write(dap, DP_CTRL_STAT, ctrlstat & (dap->dp_ctrl_stat | SSTICKYERR));
        else
...
static int amba_dbg_op_queue_run(struct adiv5_dap *dap)
{
...
/** */
static void amba_dbg_op_quit(struct adiv5_dap *dap)
{
    int retval;
...
static int amba_dbg_op_queue_dp_read(struct adiv5_dap *dap, unsigned int reg, uint32_t *data)
{
...
static int amba_dbg_op_queue_dp_write(struct adiv5_dap *dap, unsigned int reg, uint32_t data)
{
    if (stlink_dap_get_error() != ERROR_OK)
        return ERROR_OK;
...
static int amba_dbg_op_queue_ap_read(struct adiv5_ap *ap, unsigned int reg, uint32_t *data)
{
    if (stlink_dap_get_error() != ERROR_OK)
        return ERROR_OK;
    ...
    /* test STLINK_F_HAS_CSW implicitly tests STLINK_F_HAS_MEM_16BIT,
     * STLINK_F_HAS_MEM_RD_NO_INC and STLINK_F_HAS_RW_MISC */
    if ((stlink_dap_handle->version.flags & STLINK_F_HAS_CSW) &&
        (reg == ADIV5_MEM_AP_REG_DRW || reg == ADIV5_MEM_AP_REG_BD0 ||
         reg == ADIV5_MEM_AP_REG_BD1 || reg == ADIV5_MEM_AP_REG_BD2 ||
         reg == ADIV5_MEM_AP_REG_BD3)) {
        /* de-queue previous write-TAR */
        struct dap_queue *prev_q = q - 1;
        if (i && prev_q->cmd == CMD_AP_WRITE && prev_q->ap_w.ap == ap &&
            prev_q->ap_w.reg == ADIV5_MEM_AP_REG_TAR) {
        ...
        if (i && prev_q->cmd == CMD_AP_WRITE && prev_q->ap_w.ap == ap &&
            prev_q->ap_w.reg == ADIV5_MEM_AP_REG_CSW && !prev_q->ap_w.changes_csw_default) {
            stlink_dap_handle->queue_index = i;
            q                              = prev_q;
        }
        ...
        q->mem_ap.addr   = (reg == ADIV5_MEM_AP_REG_DRW) ? ap->tar_value :
                                                           ((ap->tar_value & ~0x0f) | (reg & 0x0c));
        q->mem_ap.ap     = ap;
...
static int amba_dbg_op_queue_ap_write(struct adiv5_ap *ap, unsigned int reg, uint32_t data)
{
    if (stlink_dap_get_error() != ERROR_OK)
        return ERROR_OK;
    ...
    /* test STLINK_F_HAS_CSW implicitly tests STLINK_F_HAS_MEM_16BIT,
     * STLINK_F_HAS_MEM_WR_NO_INC and STLINK_F_HAS_RW_MISC */
    if ((stlink_dap_handle->version.flags & STLINK_F_HAS_CSW) &&
        (reg == ADIV5_MEM_AP_REG_DRW || reg == ADIV5_MEM_AP_REG_BD0 ||
         reg == ADIV5_MEM_AP_REG_BD1 || reg == ADIV5_MEM_AP_REG_BD2 ||
         reg == ADIV5_MEM_AP_REG_BD3)) {
...
static int amba_dbg_config_trace(bool                   enabled,
                                 enum tpiu_pin_protocol pin_protocol,
                                 uint32_t               port_size,
                                 unsigned int          *trace_freq,
                                 unsigned int           traceclkin_freq,
                                 uint16_t              *prescaler)
{
    return stlink_config_trace(stlink_dap_handle,
                               enabled,
                               pin_protocol,
                               port_size,
                               trace_freq,
...
static int amba_dbg_trace_read(uint8_t *buf, size_t *size)
{
...
/** amba-dbg 的子命令, 比如:
 *
 * amba-dbg vid_pid 0x0483 0x3744 0x0483 0x3748 0x0483 0x374b 0x0483 0x374d
 * 0x0483 0x374e 0x0483 0x374f 0x0483 0x3752 0x0483 0x3753 0x0483 0x3754 0x0483
 * 0x3755 0x0483 0x3757 amba-dbg backend usb amba-dbg cmd 4 0x00 0x01 0x02 0x03
 * # 不确定对应什么意思
 */
static const struct command_registration amba_dbg_subcommand_handlers[] = {
    {
     .name    = "vid_pid",
...
/** */
static const struct command_registration amba_dbg_command_handlers[] = {
    {
     .name  = "amba-dbg",
     .mode  = COMMAND_ANY,
     .help  = "perform st-link management",
     .chain = amba_dbg_subcommand_handlers,
     .usage = "",
     },
    COMMAND_REGISTRATION_DONE
};
...
static int amba_dbg_init(void)
{
    ...
    if (transport_is_dapdirect_swd()) {
		...
		return dbg_bus_open(&stlink_dap_param, mode, (void **)&stlink_dap_handle);
	} else {
...
/** */
static int amba_dbg_reset(int req_trst, int req_srst)
{
    LOG_DEBUG("%d", req_srst);

    int srst = req_srst ? STLINK_DEBUG_APIV2_DRIVE_NRST_LOW : STLINK_DEBUG_APIV2_DRIVE_NRST_HIGH;

    return stlink_usb_assert_srst(stlink_dap_handle, srst);
}

/** */
static int amba_dbg_speed(int speed)
{
    if (speed == 0) {
...
/** */
static int amba_dbg_khz(int khz, int *jtag_speed)
{
    if (khz == 0) {
...
/** */
static int amba_dbg_speed_div(int speed, int *khz)
{
    *khz = speed;
    return ERROR_OK;
}

// target 会调用这里的 interface
static const struct dap_ops amba_dbg_ops = {
    .connect        = amba_dbg_op_connect,
    .send_sequence  = amba_dbg_op_send_sequence,
    .queue_dp_read  = amba_dbg_op_queue_dp_read,
    .queue_dp_write = amba_dbg_op_queue_dp_write,
    .queue_ap_read  = amba_dbg_op_queue_ap_read,
    .queue_ap_write = amba_dbg_op_queue_ap_write,
    .queue_ap_abort = amba_dbg_op_queue_ap_abort,
    .run            = amba_dbg_op_queue_run,
    .quit           = amba_dbg_op_quit,  // 可选
    .sync           = NULL,              // 可选
};

struct adapter_driver amba_debug_bus_adapter_driver = {
    .name                   = "amba-dbg",
    ...
    .commands               = amba_dbg_command_handlers,

    .init         = amba_dbg_init,
    .quit         = amba_dbg_quit,
    .reset        = amba_dbg_reset,
    .speed        = amba_dbg_speed,
    .khz          = amba_dbg_khz,
    .speed_div    = amba_dbg_speed_div,
    .config_trace = amba_dbg_config_trace,
    .poll_trace   = amba_dbg_trace_read,

    .dap_jtag_ops = &amba_dbg_ops,
    .dap_swd_ops  = &amba_dbg_ops,
    .swim_ops     = NULL,
};
